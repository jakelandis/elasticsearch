...
public class MutateTest {
    ...
    public static void readMutations(File file) throws IOException {

        YAMLParser yamlParser = yaml.createParser(file);

        MappingIterator<TestMutations> mutations =  mapper.readValues(yamlParser, TestMutations.class);
        while(mutations.hasNext()){
            System.out.println("******************** " + mutations.next());
        }
