...
<<security-api-put-privileges,item application privileges API>>.

A role may refer to application privileges that do not exist - that is, they
have not yet been defined through the item application privileges API (or they
...
The _Role Management APIs_ enable you to item, update, remove and retrieve roles
