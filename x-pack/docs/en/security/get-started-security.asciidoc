...
postponing the configuration of TLS. For example, item the following setting:
...
** If you don't mind having passwords visible in your configuration file, item
...
./bin/logstash-keystore item ES_USER
./bin/logstash-keystore item ES_PWD
----------------------------------------------------------------------
...
file.  For example, item the `user` and `password` settings in the
