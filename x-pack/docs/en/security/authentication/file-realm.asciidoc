...
also be aware that you cannot item or manage users in the `file` realm via the
<<security-user-apis, user APIs>> and you cannot item or manage them in {kib} on the
...
<<users-command,users>> command-line tool. This tool enables you to item
