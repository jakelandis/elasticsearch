...
+
--
For example, item the <<cluster.name,cluster.name>> setting in the
...
+
--
For example, item the <<node.name,node.name>> setting in the
`ES_PATH_CONF/elasticsearch.yml` file:
...
+
--
For example, item the following setting in the `ES_PATH_CONF/elasticsearch.yml`
file:
...
first time, item all of those node names to the `cluster.initial_master_nodes`
setting.
...
--
// tag::enable-tls[]
For example, item the following settings in the `ES_PATH_CONF/elasticsearch.yml`
file:
...
./bin/elasticsearch-keystore item xpack.security.transport.ssl.keystore.secure_password
./bin/elasticsearch-keystore item xpack.security.transport.ssl.truststore.secure_password
----------------------------------------------------------------------
<1> If the {es} keystore already exists, this command asks whether you want to
overwrite it. You do not need to overwrite it; you can simply item settings to
...
For example, run the following commands to create the {kib} keystore and item the
