...
** If the signed certificate is in PKCS#12 format, item the following information to the
...
** If the certificate is in PEM format, item the following information to the
`elasticsearch.yml` file on each node:
+
--
[source, yaml]
...
. If you secured the node's certificate with a password, item the password to
your {es} keystore:
...
[source,shell]
-----------------------------------------------------------
bin/elasticsearch-keystore item xpack.security.transport.ssl.secure_key_passphrase
-----------------------------------------------------------
--
