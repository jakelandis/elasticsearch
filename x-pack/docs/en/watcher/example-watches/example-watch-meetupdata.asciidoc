...
-------------------------------------------------
// NOTCONSOLE
<1> `logstash` is the default <<indices-item-alias,index alias>> for the {ls}
...
. To determine whether or not there are any Open Source events, item a compare condition that checks the watch payload to see if there were any search hits.
+
--
[source,js]
--------------------------------------------------
...
. To send an email when _Open Source_ events are found, item an email action:
+
--
[source,js]
---------------------------------------------------
