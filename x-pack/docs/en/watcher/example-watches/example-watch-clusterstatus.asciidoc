...
* <<health-item-input,Schedule the watch and define an input>> that gets the
  cluster health status.

* <<health-item-condition,Add a condition>> that evaluates the health status to
...
[discrete]
[[health-item-input]]
==== Schedule the watch and item an input
...
To load the health status into your watch, you simply item an
...
For example, you could item a condition to check to see if the status is RED.
...
For example, you could item an action to index the cluster status information
...
`elasticsearch.yml` configuration file and restart Elasticsearch. To item an email
