...
* <<log-item-input, Schedule the watch and define an input>>.
* <<log-item-condition, Add a condition>> that checks to see if an alert
needs to be sent.
...
For this compare condition to evaluate to `true`, you need to item an event
...
Once you item this event, the next time the watch executes its condition will
