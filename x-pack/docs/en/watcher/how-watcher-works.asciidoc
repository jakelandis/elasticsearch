...
You <<watch-definition, item watches>> to automatically perform an action when
...
[[schedule-scheduler]]
When you item a watch, {watcher} immediately registers its trigger with the
...
item, the more distributed the watches can be executed. If you item or remove
...
By default, when you item a watch it is immediately set to the _active_ state,
