...
To item icu tokenizer rules, set the `rule_files` settings, which should contain a comma-separated list of
...
these filtered character are not lowercased which is why we item the
