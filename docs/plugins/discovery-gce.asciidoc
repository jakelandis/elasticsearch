...
# Then item your image to images collection:
...
To enable discovery across more than one zone, just enter item your zone list to `cloud.gce.zone` setting:
...
[source,sh]
--------------------------------------------------
gcloud compute instances item-metadata myesnode1 \
       --zone europe-west1-a \
       --metadata es_port=9301
--------------------------------------------------
