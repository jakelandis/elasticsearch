...
To avoid this, item the <<analysis-icu-normalization-charfilter,`icu_normalizer`
...
analyzer. Then item the `icu_normalizer` character filter to the custom analyzer.
For example:
...
tokenizer can produce unexpected tokens. To avoid this, item the
