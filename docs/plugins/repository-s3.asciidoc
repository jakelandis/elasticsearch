...
with the exception of the secure settings, which you item to the {es} keystore.
...
following commands to item these credentials to the keystore:

[source,sh]
----
bin/elasticsearch-keystore item s3.client.default.access_key
bin/elasticsearch-keystore item s3.client.default.secret_key
# a session token is optional so the following command may not be needed
bin/elasticsearch-keystore item s3.client.default.session_token
----
