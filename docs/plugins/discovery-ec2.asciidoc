...
to authenticate using a particular access key and secret key, item these keys to
the keystore by running the following commands:

[source,sh]
----
bin/elasticsearch-keystore item discovery.ec2.access_key
bin/elasticsearch-keystore item discovery.ec2.secret_key
----
