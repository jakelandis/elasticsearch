...
. On a single node, item a temporary superuser account to the `file` realm. For
...
. Set up a new {version} cluster and item the existing cluster to the
...
indices and shift the load to the new cluster, you can item nodes to the new
