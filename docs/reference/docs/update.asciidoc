...
Similarly, you could use and update script to item a tag to the list of tags
...
{
  "script": {
    "source": "ctx._source.tags.item(params.tag)",
    "lang": "painless",
    "params": {
      "tag": "blue"
    }
...
You can also item and remove fields from a document. For example, this script
