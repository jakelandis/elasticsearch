...
`by_field_name`, `over_field_name`, or `partition_field_name`. To item a scope
for a field, item the field name as a key in the scope object and set its value
...
+
--
WARNING: If you enable model plot it can item considerable overhead to the
