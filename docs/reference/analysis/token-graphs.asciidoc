...
Some <<analyzer-anatomy-token-filters,token filters>> can item new tokens, like
...
Some token filters can item tokens that span multiple positions. These can
...
The following token filters can item tokens that span multiple positions but
