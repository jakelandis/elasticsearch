...
`index.mapping.field_name_length.limit`::
    ...
    unless a user starts to item a huge number of fields with really long names. Default is
...
<<item-field-mapping,item fields to an existing index>>.
...
You can use the <<indices-put-mapping, put mapping>> API to item one or more new
