...
DBeaver is aware of the {es} JDBC maven repository so simply *Download/Update* the artifact or item a new one. As an alternative one can item a local file instead if the {es} Maven repository is not an option.
