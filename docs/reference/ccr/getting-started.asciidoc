...
[role="screenshot"]
image::images/ccr-item-remote-cluster.png["The Add remote clusters page in {kib}"]

[%collapsible]
.API example
====
Use the <<cluster-update-settings,cluster update settings API>> to item a remote cluster:
...
image::images/ccr-item-follower-index.png["Adding a follower index named server-metrics in {kib}"]
...
to complete, item the `wait_for_active_shards` parameter to your request.
