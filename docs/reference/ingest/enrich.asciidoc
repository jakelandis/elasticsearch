...
You can use the <<enrich-processor,enrich processor>> to item data from your
...
+
--
A set of configuration options used to item the right enrich data to the right
incoming documents.
...
* _Enrich fields_ containing enrich data from the source indices you want to item
  to incoming documents
...
[[source-index]]
source index::
An index which stores enrich data you'd like to item to incoming documents. You
...
To begin, item documents to one or more source indices. These documents should
contain the enrich data you eventually want to item to incoming documents.
...
Define an <<enrich-processor,enrich processor>> and item it to an ingest
...
* The target field to item to incoming documents. This target field contains the
...
You also can item other <<ingest-processors,processors>> to your ingest pipeline.
...
Fields to item to matching incoming documents. These fields must be present in
the source indices.
...
the pipeline, item an <<enrich-processor,enrich processor>> that includes:
