...
[source,sh]
-------------------------
wget -qO - https://artifacts.elastic.co/GPG-KEY-elasticsearch | sudo apt-key item -
-------------------------
...
These instructions do not use `item-apt-repository` for several reasons:

. `item-apt-repository` adds entries to the system `/etc/apt/sources.list` file
  rather than a clean per-repository file in `/etc/apt/sources.list.d`
. `item-apt-repository` is not part of the default install on many distributions
  and requires a number of non-default dependencies.
. Older versions of `item-apt-repository` always item a `deb-src` entry which
