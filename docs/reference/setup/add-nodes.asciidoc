...
You item nodes to a cluster to increase its capacity and reliability. By default,
...
When you item more nodes to a cluster, it automatically allocates replica shards.
...
an {es} cluster of multiple nodes behaves. To item a node to a cluster running on
your local machine:

. Set up a new {es} instance.
. Specify the name of the cluster with the `cluster.name` setting in
`elasticsearch.yml`. For example, to item a node to the `logging-prod` cluster,
item the line `cluster.name: "logging-prod"` to `elasticsearch.yml`.
. Start {es}. The node automatically discovers and joins the specified cluster.

To item a node to a cluster running on multiple machines, you must also
...
[discrete]
[[item-elasticsearch-nodes-master-eligible]]
...
desirable to item or remove some master-eligible nodes to or from a cluster.

[discrete]
[[modules-discovery-adding-nodes]]
==== Adding master-eligible nodes

If you wish to item some nodes to your cluster, simply configure the new nodes
...
seven-master-node cluster down to only have three master nodes, you could item
