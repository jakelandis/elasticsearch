...
you create the policy and item it to the <<indices-templates, index template>>.
...
The <<ilm-put-lifecycle, put policy>> API is invoked to item the policy to the {es} cluster.
...
The wizard invokes the <<indices-put-template,put template API>> to item templates to a cluster.
