...
* `_all`, to item all nodes to the subset.
* `_local`, to item the local node to the subset.
* `_master`, to item the currently-elected master node to the subset.
* a node id or name, to item this node to the subset.
* an IP address or hostname, to item all matching nodes to the subset.
...
* `master:true`, `data:true`, `ingest:true`, `voting_only:true`, `ml:true`, or
  `coordinating_only:true`, which respectively item to the subset all
