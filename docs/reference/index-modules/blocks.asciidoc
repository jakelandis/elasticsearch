...
`index.blocks.read_only_allow_delete`::
    ...
    allocator>> may item and remove this block automatically.
+
...
[[item-index-block-api-path-params]]
==== {api-path-parms-title}

include::{es-repo-dir}/rest-api/common-parms.asciidoc[tag=index]
+
To item blocks to all indices, use `_all` or `*`. To disallow the adding
...
`<block>`::
(Required, string)
Block type to item to the index.
+
...
====
[discrete]
[[item-index-block-api-query-params]]
...
[discrete]
[[item-index-block-api-example]]
==== {api-examples-title}

The following example shows how to item an index block:
