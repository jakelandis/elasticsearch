...
[[item-alias-api-path-params]]
==== {api-path-parms-title}

`<index>`::
(Required, string)
Comma-separated list or wildcard expression of index names
to item to the alias.
+
To item all indices in the cluster to the alias,
use a value of `_all`.
+
...
[[item-alias-api-query-params]]
==== {api-query-parms-title}

include::{es-repo-dir}/rest-api/common-parms.asciidoc[tag=timeoutparms]


[[item-alias-api-request-body]]
...
Then item the index alias for a specific user, `user_12`:
...
to item an index alias during index creation.
