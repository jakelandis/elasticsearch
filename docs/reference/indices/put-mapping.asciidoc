...
[[item-new-field-to-object]]
===== Add new properties to an existing object field

You can use the put mapping API to item new properties to an existing
...
Use the put mapping API to item a new inner `last` text field to the `name`
field.
...
[[item-multi-fields-existing-field-ex]]
...
Instead, item an <<alias, `alias`>> field to create an alternate field name.
// end::rename-field[]
...
Use the put mapping API to item the `user_id` field alias
