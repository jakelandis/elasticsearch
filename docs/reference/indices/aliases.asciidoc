...
{
  "actions" : [
    { "item" : { "index" : "my-index-000001", "alias" : "alias1" } }
  ]
}
----
// TEST[setup:my_index]
...
+
NOTE: You cannot item <<data-streams,data streams>> to an index alias.
...
item, remove, or delete.
+
If the `aliases` parameter is not specified,
this parameter is required for the `item` or `remove` action.
...
[[indices-aliases-api-item-alias-ex]]
===== Add an alias
...
Renaming an alias is a simple `remove` then `item` operation within the
...
POST /_aliases
{
  "actions" : [
    { "remove" : { "index" : "test1", "alias" : "alias1" } },
    { "item" : { "index" : "test1", "alias" : "alias2" } }
  ]
}
--------------------------------------------------
// TEST[continued]

[[indices-aliases-api-item-multi-alias-ex]]
===== Add an alias to multiple indices

Associating an alias with more than one index is simply several `item`
actions:
...
{
  "actions" : [
    { "item" : { "indices" : ["test1", "test2"], "alias" : "alias1" } }
  ]
}
--------------------------------------------------
// TEST[s/^/PUT test1\nPUT test2\n/]
...
{
  "actions" : [
    { "item" : { "index" : "test*", "alias" : "all_test_indices" } }
  ]
}
--------------------------------------------------
// TEST[s/^/PUT test1\nPUT test2\n/]
