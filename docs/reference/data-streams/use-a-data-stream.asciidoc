...
* <<item-documents-to-a-data-stream>>
...
You can item documents to a data stream using two types of indexing requests:
...
You cannot item new documents to a stream's other backing indices, even by
...
<<docs-index-api-op_type,`op_type`>> of `create` to item individual documents
to a data stream.
...
IMPORTANT: You cannot item new documents to a data stream using the index API's
...
You can use the <<docs-bulk,bulk API>> to item multiple documents to a data
...
means a reindex can only item new documents to a data stream. It cannot update
...
{
  "actions" : [
    { "item" : { "index" : "archive_1", "alias" : "archive" } },
    { "item" : { "index" : "archive_2", "alias" : "archive", "is_write_index" : true} }
  ]
}
----
////
