...
* <<item-new-field-mapping-to-a-data-stream>>
...
performing a reindex, you can use the same process to item new field
...
To item a mapping for a new field to a data stream, following these steps:
...
. Use the <<indices-put-mapping,put mapping API>> to item the new field mapping
...
--
+
To item the mapping only to the stream's write index, set the put mapping API's
