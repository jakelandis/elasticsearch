...
In order to start a node with a different max heap space item: `-Dtests.heap.size=4G`
In order to disable assertions item: `-Dtests.asserts=false`
...
build. To item your plugin, create a directory called elasticsearch-extra as
